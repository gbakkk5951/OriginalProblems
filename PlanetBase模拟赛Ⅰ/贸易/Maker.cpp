...
void make(){
	...
	for(I=beg;I<end;I++){
		...
		if (I == 0) {
			n = 5000;
			for (int i = 1; i <= n; i++) {
				cout << lrand(0, 100) << sp << lrand(0, 100) << sp;
			}
			cout << endl;
			for (int j = 1; j <= m; j++) {
				int op = lrand(1, 4);
			...
			n = 1;
			for (int i = 1; i <= n; i++) {
				cout << lrand(0, 1e9) << sp << lrand(0, 1e5) << sp;
			}
			cout << endl;
			for (int j = 1; j <= m; j++) {
				int op = lrand(1, 4);
				cout << op << sp;
				if (op == 1) {
					cout << bit_rand(0, 1e13);
				} else 
				if (op == 2) {
					cout << lrand(0, 1e3);
				} else 
				if (op == 3) {
					cout << lrand(1, n) << sp << lrand(0, 1e5);
				} else 
				if (op == 4) {
					cout << lrand(1, n) << sp << lrand(0, 1e9);
				}
				cout << endl;
			}
			goto EndFor1;
			...
			//make the value full
			for (int i = 1; i <= n; i++) {
				cout << lrand(1e8, 1e9) << sp << lrand(1e4, 1e5) << endl;
			}
		}
