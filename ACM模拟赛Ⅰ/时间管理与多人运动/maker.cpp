...
void make(){
	...
	for(I=beg;I<end;I++){
		...
		for (int i = 1; i <= n; i++) {
			for (int j = 1; j <= n; j++) {
				if (j <= i && j <= 10 * 70) {
					cout << max(0, 70 - (j + rand() % 1) / 10 ) << sp;
				} else {
					cout << (((rand() & 256) == 3) ? lrand(0, mxh) : lrand(0, min(3, (n - j) / 100))) << sp;
				}
				/*
				if (rand() % 10 == 1) cout << ((rand() % 4 == 0) ? bit_rand(0, mxh) : lrand(0, mxh / 4)) << sp;
				else cout << (j  > n / 4 ? lrand(0, mxh) : lrand(0, mxh / 4)) << sp;
				*/
			}
			cout << endl;
		}
