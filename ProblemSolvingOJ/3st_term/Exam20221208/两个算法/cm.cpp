...
void make(){
	...
	for(I=beg;I<end;I++){
		...
		cout << V << sp << E << sp << Q << endl;
		for (int i = 1; i <= E; ++i)
		{
			int l, r;
			if (bIsGraphConnected)
			{
				l = 1;
				r = V;
			} 
			...
			cout << lrand(l, r) << sp << lrand(l, r) << sp << (rand() % 1000 ? lrand(0, 1e9) : bit_rand(0, 1e9)) << endl;
		}
		for (int i = 1; i <= Q; ++i)
		{
			cout << lrand(1, V) << sp << lrand(1, V) << endl;
		}
		EndFor1:
		cout.close();
	}
