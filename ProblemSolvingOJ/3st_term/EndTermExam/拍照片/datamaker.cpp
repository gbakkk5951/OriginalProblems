...
void make(){
	...
	for(I=beg;I<end;I++){
		...
		int CaseNum = 10;
		int MaxVertexNum = 20, MaxEdgeNum = 20;
		if (I == -2)
		{
			CaseNum = 3;
			MaxVertexNum = 7, MaxEdgeNum = 10;
		}
		else if (I == 1)
		{	
			CaseNum = 1;
		...
		random_shuffle(TypeArray.begin(), TypeArray.end());
		for (int CaseCnt = 1; CaseCnt <= CaseNum; ++CaseCnt)
		{
			int CaseType = TypeArray[CaseCnt - 1];
			int ENum = MaxEdgeNum - (rand() & 1), VNum = MaxVertexNum - (rand() & 1);
			Edges.clear();
			if (CaseType == 1)
			{
				//Chain
			...
			else if (CaseType == 5)
			{
				...
				for (int i = 2; i <= VNum; ++i)
				{
					Edges.push_back((p) {lrand(1, i - 1), i});
				}
			}
			else if (CaseType == 6)
			{
				...
				while (Edges.size() < ENum)
				{
					Edges.push_back((p) {lrand(1, VNum), lrand(1, VNum)});
				}
			}
			else if (CaseType == 7)
			{
			...
			{
				//Two Chains + Legal Edges

				for (int i = 2; i <= VNum / 2; ++i)
				{
					Edges.push_back((p) {i - 1, i});
				}
				for (int i = VNum / 2 + 2; i <= VNum; ++i)
				{
					Edges.push_back((p) {i - 1, i});
				}
				while (Edges.size() < ENum)
			...
			else if (CaseType == 10)
			{
				...
				int S = lrand(1, VNum);
				Edges.push_back((p) {S, S});
			}
			
			cout << VNum << sp << ENum << endl;
			...
			for (auto Edge : Edges)
			{
				cout << Name[Edge.first] << sp << Name[Edge.second] << endl;
			}
