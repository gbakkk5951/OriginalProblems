...
List<MaxV, MaxV> ChildQueue, ChildSelectedNode;
...
void GetScheme()
{
	...
	{
		ChildQueue.Insert(ChildSelectedNode.Size[i], i);
	}
	int QueueIndex1 = MaxChildSelectedSize;
	int QueueIndex2 = QueueIndex1;
	for (int i = 1; i <= PairNum; ++i)
	{	
		while (ChildQueue.Size[QueueIndex1] == 0)
		{
			--QueueIndex1;
		}
		while (ChildQueue.Size[QueueIndex2] == 0 || QueueIndex2 == QueueIndex1 && ChildQueue.Size[QueueIndex2] == 1)
		{
			--QueueIndex2;
		}
		int ItemId1 = ChildQueue.PopHead(QueueIndex1);
		int ItemId2 = ChildQueue.PopHead(QueueIndex2);
		int ChildId1 = ChildQueue.Items[ItemId1].Val;
		int ChildId2 = ChildQueue.Items[ItemId2].Val;
		...
		ChildQueue.Transplant(QueueIndex1 - 1, ItemId1);
		ChildQueue.Transplant(QueueIndex2 - 1, ItemId2);
	}
}
