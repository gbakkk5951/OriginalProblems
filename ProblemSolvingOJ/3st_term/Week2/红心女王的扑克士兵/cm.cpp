...
void make(){
	...
	for(I=beg;I<end;I++){
		...
		{
			n = 600 + I * 200 + lrand(0, 200);
		} 
		else
		{
			n = I == 9 ? 1e6 : (lrand(5e5, 1e6));
		}
		cout << n << endl;
		Soldiers.clear();
		lld OrderNum = n / 3;
		lld NonOrderNum = n / 3;// n / 3;
		lld RandomNum = n - OrderNum - NonOrderNum;
		if (OrderNum)
		{
			lld BaseX = lrand(-MaxValue, MaxValue / 2);
			lld BaseY = lrand(-MaxValue, MaxValue / 2);
			lld MaxDeltaX = (MaxValue - BaseX) / OrderNum;
			lld MaxDeltaY = (MaxValue - BaseY) / OrderNum;
			for (lld i = 0; i < OrderNum; ++i)
			{
				Soldiers.push_back(make_pair(BaseX, BaseY));
				BaseX += lrand(0, MaxDeltaX);
				BaseY += lrand(0, MaxDeltaY);
			}
		}
		if (NonOrderNum)
		{
			lld BaseX = lrand(-MaxValue, MaxValue / 2);
			lld BaseY = lrand(-MaxValue / 2, MaxValue);
			lld MaxDeltaX = (MaxValue - BaseX) / NonOrderNum;
			lld MaxDeltaY = (BaseY - (-MaxValue)) / NonOrderNum;
			for (lld i = 0; i < NonOrderNum; ++i)
			{
				Soldiers.push_back(make_pair(BaseX, BaseY));
				BaseX += lrand(0, MaxDeltaX);
				BaseY -= lrand(0, MaxDeltaY);
		...
		if (RandomNum)
		{
			for (lld i = 0; i < RandomNum; ++i)
			{
				Soldiers.push_back(make_pair(lrand(-MaxValue, MaxValue), lrand(-MaxValue, MaxValue)));
			}
		}
		for (lld i = 1; i < Soldiers.size(); ++i)
		{
			swap(Soldiers[i], Soldiers[lrand(0, i)]);
		}
		for (auto Soldier : Soldiers)
		{
			cout << Soldier.first << sp << Soldier.second << endl;
		}
		EndFor1:
		cout.close();
	}
