...
void make(){
	...
	for(I=beg;I<end;I++){
		...
		limit = (((1 << (rand() % 31)) - 1) & (rand())) % (int) (1e9) + 1;
		if (I % 10 == 5) {
			limit = 892371480 - 1;
		} 
		cout << n << sp << m << sp << limit << endl;
		for (int i = 1; i <= n; ++i) {
			if (I % 5 == 0 && i > 300 || rand() % 27 == 1 && i > 200) {
				cout << 892371480;
			} else {
				cout << lrand(1, 1e9);
			}
	...
	for(I=exbeg;I<exend;I++){
		...
		if (I == 3){
			...
			for (int i = 1; i <= n; ++i) {
				if (i <= m)
				cout << 31019 * (31019 - 500 + i);
				else cout << lrand(1, 31019 - 1);
				cout << sp;
			}	
		}
