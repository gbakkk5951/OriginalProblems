...
void make(){
	...
	for(I=beg;I<end;I++){
		...
		long long n = (I < 10 ? lrand(1, 100) : 1.8e7);
		int mod = (I & 1 ? lrand(-3, 3) : lrand(-1e9, 1e9));
		mod += !mod;
		lld A = lrand(-1e9, 1e9), B = lrand(-1e9, 1e9), C = lrand(-1e9, 1e9);
		mod += mod == 1;
		if (I == 7) {
			mod = 1;
		} else
		...
		cout << n << sp << A <<  sp << B << sp << C << sp << mod << endl;
		lld neg = 0, pos = 0;
		for (int i = 1; i <= n; ++i) {
			A = C + A * B % mod;
			if (A < 0) neg += A;
	...
	for(I=exbeg;I<exend;I++){
		...
		long long n = 4e5;
		int lim = 7;
		int mod = (I & 1 ? lrand(-lim, lim) : lrand(-lim, lim));
		mod += !mod;
		lld A = lrand(-lim, lim), B = lrand(-lim, lim), C = lrand(-lim, lim);
		
		
		lld neg = 0, pos = 0;
//		A = 686090931, B = -816338550, C = -5, mod = 73499;
		A = -719095888, B =  -742107893, C = 27, mod = 100177;
		cout << n << sp << A <<  sp << B << sp << C << sp << mod << endl;
