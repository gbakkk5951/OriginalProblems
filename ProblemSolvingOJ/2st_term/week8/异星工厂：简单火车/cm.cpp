...
void make(){
	...
	for(I=beg;I<end;I++){
		...
		int n = (I < 5 ? 2000 : 1e6), m = (I < 5 ? 3000 : 2e6);
		cout << n << sp << m << endl;
		if (n % 3 == 0) m = n - 1;
		if (I < 10) for (int i = 2; i <= n; ++i) {
			cout << lrand(1, i - 1) << sp << i << sp << lrand(-1e9, 1e9) << endl;
			--m;
		}
		while (m--) {
			cout << lrand(1, n) << sp << lrand(1, n) << sp << lrand(-1e9, 1e9) << endl;
		}
	...
	for(I=exbeg;I<exend;I++){
		...
		cout << n << sp << m << endl;
		for (int i = 1; i <= m; ++i) {
			cout << 1 << sp << 1 << sp << lrand(-1e9, 1e9) << endl;
		}
