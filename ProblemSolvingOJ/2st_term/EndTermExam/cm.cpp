...
lld init[maxn], cons[maxn];
void make(){
	int I;
	int i,j,k;
	for(I=beg;I<end;I++){// 0-> -1, 1-> 全都是x,0, 2->0,0; 3-> 都是非0但不会没电
		if (I == 13) continue;
		//-1在m-5的位置堆了m-4个需求
		...
		if (I == 0) {
			for (int i = 1; i <= n; ++i) {
				cons[i] = bit_rand(mnb, mxb);
			}
			for (int i = 1; i <= m - 4; ++i) {
				init[i] = cons[i] * (lld) (m - 5 - 1) + lrand(0LL, max(0LL, cons[i] - 1));
			} 
			for (int i = m - 3; i <= n; ++i) {
				init[i] = cons[i] * (lld) (m + 2) + lrand(0LL, max(0LL, cons[i] - 1));
			}
		
		} else
		if (I == 1) {
			for (int i = 1; i <= n; ++i) {
				init[i] = bit_rand(mnb, mxb);
				cons[i] = 0;
			}
		...
		if (I == 3) {
			...
			for (int i = 1; i <= n; ++i) {
				init[i] = cons[i] * (lld) (m) + (rand() & 1 ? lrand(0, max(0LL, cons[i] - 1)) : 0);
			}
		} else {
			for (int i = 1; i <= n; ++i) {
				cons[i] = bit_rand(mnb, mxb);
			...
			for (int i = 1; i <= choose; ++i) {
				if (I & 1) {//原地
					init[occu[i]] = cons[occu[i]] * (lld) (i - 1) + lrand(0, max(0LL, cons[occu[i]] - 1));
				} else {//区间
					init[occu[i]] = cons[occu[i]] * (lld) (lrand(i, m) - 1) + lrand(0, max(0LL, cons[occu[i]] - 1));
				}
			}
			for (int i = choose + 1; i <= n; ++i) {
				init[occu[i]] = lrand(cons[occu[i]] * (lld)m, (lld)1e16);
			}
