...
void make(){
	...
	for(I=beg;I<end;I++){
		...
		if (I >= 16) {
			for (int i = 2; i <= n; ++i) {
				int cho = a[i - lrand(1, min(i - 1, 100))];
				while (d[cho] && r[cho]) cho = a[i - lrand(1, min(i - 1, 100))];
				if (r[cho] || lrand(0, 1) && !d[cho]) {
					d[cho] = a[i];
				} else {
					r[cho] = a[i];
				}
		...
		if (I % 4 > 1) {
			for (int i = 2; i <= n; ++i) {
				int cho = a[lrand(1, i - 1)];
				while (d[cho] && r[cho]) cho = a[i - lrand(1, i - 1)];
				if (r[cho] || lrand(0, 1) && !d[cho]) {
					d[cho] = a[i];
				} else {
					r[cho] = a[i];
...
lld bit_rand() {
    return llrand() & ((1llu << lrand(0, 20)) - 1);
} 
