...
void make(){
	...
	for(I=beg;I<end;I++){
		...
		if (I < 3) {
			...
			for (int i = 1; i <= m; ++i) {
				int l, r;
				randRange(1, (int)1e3, l, r);
				cout << l << sp << r << sp << a[lrand(0, 25)] << endl;
			}
		} else
		if (I < 8) {
			for (int i = 1; i <= 250; ++i) {
			...
			string ten = {a[0], a[1], a[0], a[0], a[0], a[1], a[0], a[0], a[1], a[0]};
			for (int i = 1; i <= 50; ++i) {
				cout << ten;
				S += ten;
			}
			...
			for (int i = 1; i <= m; ++i) {
				...
				if (i % 3) {
					...
					thislen = 1 + (lrand(1, slen - 1) & ((1 << lrand(1, 6)) - 1));
					if (i % 7 == 0) cout << S.substr(lrand(0, len - thislen), thislen) << a[lrand(0, 7)];
					else cout << S.substr(lrand(0, len - thislen), thislen);
					cout << endl;
				} else {
					thislen = 1 + (lrand(1, slen - 1) & ((1 << lrand(1, 6)) - 1));
					for (int i = 1; i <= thislen; ++i) cout << a[rand() & 1];
					cout << endl;
				}
			}
		...
		if (I == 8) {
			...
			for (int i = 1; i <= m; ++i) {
				...
				cout << l << sp << r << sp;
				if (i & 1) cout << S.substr(lrand(0, len - thislen), thislen) << a[lrand(0, 7)];
				else cout << S.substr(lrand(0, len - thislen), thislen);
				cout << endl;
			}
		} else
		if (I == 9) {
			...
			for (int i = 1; i <= m; ++i) {
				...
				bit_randRange(1, len, l, r);
				cout << l << sp << r << sp;
				int thislen = bit_rand(1, slen);
				if (I % 3) {
					if (I % 5 < 2) thislen = lrand(1, 3);
					for (int i = 1; i <= thislen; ++i) {
						cout << a[lrand(0, 25)];
					}
				} else {
					cout << S.substr(lrand(0, len - thislen), thislen);
				}
				cout << endl;
			}
