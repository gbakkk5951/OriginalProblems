...
int newid[MXN];
void getids(int n, int sh = 1) {
	for (int i = 1; i <= n; ++i) newid[i] = i;
	if (sh) shuffle(newid + 1, n);
}
void make(){
	int I;
	int i,j,k;
	for(I=beg;I<end;I++){
		...
		if (I < 2) {//15随机
			...
			++tn;
			for (int i = 0; i < 3; i++) {
				d[tn].push_back(lrand(1, 3));
			}
			d.resize(d.size() + 1);
			++tn;
			for (int i = 0; i < 2; i++) {
				d[tn].push_back(lrand(1, 2));
			}
			d.resize(d.size() + 1);
			++tn;
			for (int i = 0; i < 4; i++) {
				d[tn].push_back(lrand(1, 4));
			}
		} else
		if (I == 3) {//遍历1到6
			for (int len = 1; len <= 6; ++len) {
				...
				while (1) {
					int flag = 0;
					for (int j = 0; j < len; ++j) if (nowd[j] < len) {
						flag = 1;
						nowd[j]++;
						for (int k = j - 1; k >= 0; --k) nowd[k] = 1;
						break;
					}
					if (!flag) break;
					d.resize(d.size() + 1);
					++tn;
					for (int i = 0; i < len; i++) {
						d[tn].push_back(nowd[i]);
					}
				}
			}
		
		} else
		if (I == 4) { // 7\8\9\10\11 随机
			while (rem >= 11) {
				...
				rem -= len;
				for (int i = 0; i < len; ++i) {
					d[tn].push_back(lrand(1, len));
				}
			}
		} else
		if (I < 7) {// 2000 ~ 5000 随机, 内容有构造
			int cnt = 0;
			while (rem > 0) {
				if (rem < 100) {
					len = rem;
				...
				getids(len);
				if (cnt == 1) {
					for (int i = 1; i <= len; ++i) {//邻居随机
						int t = i + lrand(-1, 1);
						if (t == len + 1) t = 1;
						else if (t == 0) t = len;
						d[tn][newid[i] - 1] = newid[t];
					}
				} else
				if (cnt % 7 == 0) { // 对环
					for (int i = 1; i <= len; ++i) {//邻居随机
				...
				if ( cnt % 13 == 0) {//随机
					...
					d[tn][newid[1] - 1] = 1;
					for (int i = 2; i <= len; ++i) {
						d[tn][newid[i] - 1] = newid[lrand(1, i - 1)];
					}
				}
			}
		
		} else 
		if (I < 10) { // 200000 开始 /2 直到 < 3
			int len = 200000;
			int cnt = 0;
			while (len >= 3) {
				++cnt;
		...
		if (I == 10) {// 随机1 到3, 小概率4到105;
			while (rem >= 107) {
			
				d.resize(d.size() + 1);
				++tn;
				int len = lrand(0, 1007) ? lrand(1, 3) : lrand(4, 105);
				rem -= len;
				for (int i = 0; i < len; ++i) {
					d[tn].push_back(lrand(1, len));
				}
