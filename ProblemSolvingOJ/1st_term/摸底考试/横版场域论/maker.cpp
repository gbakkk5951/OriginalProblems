...
void make(){
	...
	for(I=beg;I<end;I++){
		...
		if (I < 7) {
			...
			lim = 1e9;
			// I \in [7, 9]
			n = I == 7 ? 3e4 : rand() % (int)2e4 + 1e4, m = 3e4;
		}
		cout << n << sp << m << endl;
		for (int i = 1; i <= n; ++i) {
			cout << (dir[i] = (rand() & 1)) << sp << (char)('A' + rand() % 26) << endl;
		}
		if (I < 5) {
			int now = 1;
			for (int i = 1; i <= m; ++i) {
				int di = rand() & 1;
				int li = (dir[now] ^ di) ? n - now + 1 : now;
				int p = rand() % li;
				cout << di << sp << p << endl;
				now = now + p * ((dir[now] ^ di) ? 1 : -1);
				assert(1 <= now && now <= n);
			}
			
		} else {
			for (int i = 1; i <= m; ++i) {
				cout << ((rand() & 1)) << sp << lrand(0, lim) << endl;
			}
		}
