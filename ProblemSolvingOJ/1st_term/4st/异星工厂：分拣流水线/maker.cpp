...
void make(){
	...
	for(I=beg;I<end;I++){
		...
		if (I < 5) {
			...
			len = 1e5 - ((I & 1) && I != 7 ? lrand(0, 5000) : 0);
			max = 1e6;
			bitlen = 9;
		}
		string S;
		...
		if (I == 1) {
			for (int i = 0; i < len; ++i) {
				cout << (char) ((rand() & 1) ? 'a' + lrand(0, 25) : 'A' + lrand(0, 25));
			}
		} else
		if (I == 7) {
			while (S.length() - len > 6 + 1) {
				S += to_string(lrand(max / 2, max - 1));
				S += (char) ((rand() & 1) ? 'a' + lrand(0, 25) : 'A' + lrand(0, 25));
			}
		} else
		if (I == 8) {
			while (S.length() - len > 9 + 1) {
				S += to_string(llrand(max * 50, max * 100 - 1) / 100.0);
				S += (char) ((rand() & 1) ? 'a' + lrand(0, 25) : 'A' + lrand(0, 25));
			}
		} else
		while (S.length() < len) {
			if (S.length() - len < bitlen || last == 0 || (lrand(0, 3))) {
				S += (char) ((rand() & 1) ? 'a' + lrand(0, 25) : 'A' + lrand(0, 25));
				last = 1;
			} else {
				if ((rand() & 1)) {
					S += to_string(lrand(0, max - 1));
				} else {
					if (I >= 5 && (rand() % 13 == 1)) {
						if (rand() & 1) {
							S += to_string(lrand(0, max - 1));
							...
							S += '.';
							S += to_string(lrand(0, 99));
						}
					} else {
						S += to_string(llrand(0, max * 100 - 1) / 100.0);
//						lld a = llrand(0, max * 100 - 1);
//						cerr << endl << a << endl;
//						cerr << to_string((double)a / 100.0) << endl;
					}
				}
				last = 0;
			}
