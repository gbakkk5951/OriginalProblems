...
void make(){
	...
	for(I=beg;I<end;I++){
		...
		char bi[] = {'+', '-', '*', '<', '>', '/', '%'};
		char si[] = {'-', '+', '~'};
		int bn = sizeof(bi) / sizeof(bi[0]);
		int sn = sizeof(si) / sizeof(bi[0]);
		int cnt = 2e5 / len;
		cout << cnt << endl;
		while (cnt--) {
			int can_bi = 0;
			...
			while (used < len) {
				if (used + 1 == len) {
					if (can_num) {
						cout << (last_out = (char)(lrand(last_bi == '/' ? 1 : 0, 9) + '0'));
						++used;
					} else {
						break;
					}
				...
				if (can_bi) {
					if (inbr && last_out != '(' && lrand(0, 3)) {
						cout << (last_out = ')');
						inbr--;
					} else {
						++used;
						cout << (last_out = (char)(last_bi = bi[lrand(0, (((I % 3) && (cnt % 7 != 6)) ? bn - 3 : bn - 1) - (((I % 4) && cnt % 3 != 2) ? 2 : 0))]));
						can_bi = 0;
						can_num = 1;
					}
				} else {
					if (used + 2 >= len ||  lrand(0, (cnt & 1) ? 3 : 15)) {
						if (lrand(0, (cnt % 5) ? 1: 3)) {
							if ((I & 1) || cnt % 7 && (last_bi == '%' ||  last_bi == '/' || last_bi == '*')) {
								cout << (last_out = (char)('0' + lrand(1, 9)));
							} else {
								cout << (last_out = (char)('0' + lrand(0, 9)));
							}
							++used;
							can_num = 0;
							can_bi = 1;
