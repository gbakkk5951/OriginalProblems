	...
	for(I=beg;I<end;I++){
		...
		if (I == 16) {
			for (int i = 0; i < (int)2e3; ++i) {
				A[i] = (i & 1) ? 0 : (1 + (rand() % 5 != 0));
			}
		}
		int rn = 0;
		for (int i = 0, t; i < n + len; i += t + 1) {
			t = strlen(A + i);
			if (t > 0) {
				m[++rn] = t;
				st[rn] = i, 7;
		...
		for (int Q = 1; Q <= q; ++Q) {
			...
			if (op == 1) {
				...
				cout << x1 << sp << y1 << sp << v << sp << c << endl;
			} else
			if (op == 2) {
				int x1, y1, x2, y2;
				if (Q == 37) {
					x1 = x2 = lrand(0, rn - 1);
					y1 = y2 = lrand(0, m[x1 + 1] - 1);
				} else
				if (Q == 47) {
					x1 = x2 = lrand(0, rn - 1);
					y1 = y2 = -1;
				...
				if (Q == 57) {
					...
					x1 = lrand(0, rn - 1), x2 = lrand(0, rn - 1);
					if (rand() % 3 == 0 && I == 20) {
						y1 = lrand(-1e9, 1e9);
					} else
					if (rand() % 17) {
