...
//						shuffle(tmp + 1, gap);
				...
				if (i == gap * 2 + 18) {
					...
					static int lastl = cnt - 1, lastr = cnt;
					static int lc = 0, rc = 0;
					int rem = (i - (gap * 2 + 19)) % 7;
					if (rem == 0) {
						lc = ++cnt;
						cout << "3 1" << endl;
					} else
				...
				if (op == 5) {
					cout << op << " " << id << " " << (rand() % 10 ? lrand(1, cnt) : lrand(-1e9, 1e9)) << endl;
				} else {
					++cnt;
					cout << op << " " << lrand(1, n) << endl;
				}
