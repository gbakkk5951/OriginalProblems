...
void make(){
	...
	for(I=beg;I<end;I++){
		...
		int tp = I >> 1;
		if (tp == 0) {
			mchain(1, 2, 1700, cout);
			rand_edge(1701, n, cout);
		} else
		if (tp == 1) {
			mtree(1, 2, 1700, cout);
			rand_edge(1701, n, cout);
		...
		if (tp == 2) {
			mbtree(1, 2, 1700, cout);
			rand_edge(1701, n, cout);
		} else
		if (tp == 3) {
			mflower(1, 2, 1700, cout);
			rand_edge(1701, n, cout);
		} else
		if (tp == 4) {
			mworm(1, 2, 1700, cout);
		...
		if (tp == 5) {
			mflower(1, 2, 500, cout);
			mchain(500, 501, 1300, cout);
			mflower(1300, 1301, 1700, cout);
			rand_edge(1701, n, cout);
		} else
		if (tp == 6 || tp == 7) {
			for (int i = 0; i < 10; i++) {
				mworm(1, i * 100 + 1 + (i == 0), (i + 1) * 100, cout);
			}
			for (int i = 10; i < 15; i++) {
				mtree(lrand(1, 1000), i * 100 + 1, (i + 1) * 100, cout);
			}
			mflower(1, 1501, 1600, cout);
			mflower(lrand(500, 1500), 1601, 1700, cout);
			rand_edge(1701, n, cout);
		}
		EndFor1:
		cout.close();
...
void add(int a, int b, ostream &cout) { //要求编号较大的点是第一次连边 
	if (rand() & 1) swap(a, b);
	cout << a << sp << b << sp << lrand(0, mx)<<endl;
}
