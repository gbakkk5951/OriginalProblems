...
template<typename Type>
	__inline void read(Type &a){
		cerr<< __FUNCTION__ <<" at "<< __LINE__ <<endl;
		char t,f=1;
...
long long addpersec;
_Main(){
	cout<< __FUNCTION__ <<" at "<< __LINE__ <<endl;
    freopen("ypa3.in","r",stdin);
	int i;long long othersum,temp;int I;
	...
	for(I=(1<<knum)-1;I>0;I--)if(dp[I]>0){
		...
		for(i=0;i<knum;i++)if((I>>i)&1){//枚举干掉哪个 
			#define allatk (othersum-killing[i].atk*killing[i].cnt)
