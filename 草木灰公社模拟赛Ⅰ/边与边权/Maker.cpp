...
string stdName = "eaw_speed";
string bruteName = "eaw";
bool make_data = 0;
bool run_ans = true;
...
void make(){
	...
	for(I=beg;I<end;I++){
		...
		if (I >> 1 == 0) {// 0 ~ 7 始终， lrand 0 ~ 2; 
			...
			goto EndFor1;
		} else if (I >> 1 == 1) {// 改完恢复， 初始全 > 1 
			n -= I & 1;
			cout << n << sp << Qn << endl;
			for (int i = 1; i <= n; i++) {
				
				cout << (arr[i] = lrand(2, 100)) << sp;
			}
			cout << endl;
			for (int Q = 1; Q <= Qn; Q += 2) {
				int op = lrand(1, 3);
				int v = lrand(0, 1);
				int pos = lrand(1, n + (op == INS));
				int lst = arr[pos];
				if (op == CHANGE) {
					cout << op << sp << pos << sp << v << endl;
					cout << op << sp << pos << sp << lst << endl;
				} else if (op == INS) {
					cout << op << sp << pos << sp << v << endl;
					cout << DEL << sp << pos << endl;
				} else if (op == DEL) {
					cout << DEL << sp << pos << endl;
					cout << INS << sp << pos << sp << lst << endl;
				}
			}
		} else if (I >> 1 == 2) { // 1和n + 1处 加东西， 300 * (加够100次就左右各删100个) + 1w * 随机， 初始 >= 1 
			n -= I & 1;
			cout << n << sp << Qn << endl;
			for (int i = 1; i <= n; i++) {
				cout << (arr[i] = lrand(1, 100)) << sp;
			}
			cout << endl;
			for (int g = 1; g <= 300; g++) {
				for (int i = 1; i <= 100; i++) {
					op = n ? lrand(1, 2) : INS;
					if (i & 1) {
						cout << op << sp << lrand(1, 100);
					} else {
						cout << op << sp << lrand(n - 100 + 1, n);
					}
					if (op != DEL) {
						cout << sp << lrand(0, 100);
					}
		...
		} else if (I >> 1 == 3) { // 1000 * (1,n封住 + 96 * ([2, 100] 或 [n / 2, n / 2 + 100] 或 [n - 100]) 随机) // 初始 >= 1 
			n = 100000; Qn = 100000 - 1000;
			n -= I & 1;
			cout << n << sp << Qn << endl;
			...
			cout << endl;
			for (int g = 1; g <= 990; g++) {
				cout << CHANGE << sp << 1 << sp << 0 << endl;
				cout << CHANGE << sp << n << sp << 0 << endl;
				int are = lrand(1, 3);
				int l, r;
				if (are == 1) {
					l = 2, r = 100;
				...
				cout << CHANGE << sp << 1 << sp << lrand(2, 100) << endl;
				cout << CHANGE << sp << n << sp << lrand(2, 100) << endl;						
			}
