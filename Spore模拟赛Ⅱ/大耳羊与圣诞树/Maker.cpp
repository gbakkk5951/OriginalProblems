...
void make(){
	...
	for(I=beg;I<end;I++){
		...
		if (I == 7) {
			cout << n << sp << id[1] << endl;
			mchain(1, 2, 1e5, cout);
			mchain(1, 1e5 + 1, 2e5, cout);
			mflower(1e5, 2e5 + 1, 3.5e5, cout);
			mflower(2e5, 3.5e5 + 1, 5e5, cout);
		} else
		if (I == 8) {
			cout << n << sp << id[1] << endl;
			mksq(1, 2, n, cout);
		...
		if (I == 10) {
			cout << n << sp << id[1] << endl;
			mflower(1, 2, 3e5, cout);
			mbtree(1, 3e5 + 1, n, cout);
		}
		EndFor1:
		cout.close();
	}
