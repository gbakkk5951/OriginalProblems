...
void make(){
	...
	for(I=beg;I<end;I++){
		...
		if (I == 4) {
			...
			for (int i = 20000 + 1; i <= 70000; i++) {
				cout << 0;
			}
			for (int i = 70000 + 1; i <= 100000; i++) {
				cout << 1;
			}
		} else
		if (I == 5) {
		...
		if (I == 7) {
			...
			for (int i = 1; i <= 5; i++) {
				getrev(str, str + (3125 << i - 1), 3125 << i - 1);
			}
			for (int i = 1; i <= n; i++) {
				cout << (int)str[i];
			}
